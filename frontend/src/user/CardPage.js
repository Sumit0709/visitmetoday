...
import { createCard, getCard, getCardFromSessionId, getReferalAndVisitCounter, updateCard } from '../apiCalls/profile'
...
const CardPage = () => {
  ...
  const onGetReferalAndVisitCounter = () => {
    ...
    getReferalAndVisitCounter(token,sessionId)
    .then(res => {
      if(res.success){
  ...
  const onGetCard = (referalCode, visitCounter, userName) => {
    ...
    getCardFromSessionId(sessionId)
        .then(blob => {
