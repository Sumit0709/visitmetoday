...
exports.GenerateSharingImage = async (req, res) => {
    ...
    // NAME and verified
    const verified = userRole==process.env.USER_ROLE? await Canvas.loadImage('./media/verify.png'):(
                      userRole==process.env.ADMIN_ROLE? await Canvas.loadImage('./media/Admin2.png'):
                      await Canvas.loadImage('./media/Admin1.png'))
    ...
    canvas.toBuffer((err,buf)=>{
      ...
      Profile.findByIdAndUpdate(profileId,{sharingImage: buf},{upsert:false, new: true})
        .exec()
        .then(result => {
          if(result){
